...
# Full replacement for CalendarService, which now depends on Dio / app config / session controller

replacement = """import 'package:dio/dio.dart';
...
class CalendarService {
  ...
  Future<List<TimeEntry>> getTeamTimeEntries(DateTime from, DateTime to) async {
    try {
      final response = await _dio.get('/api/v1/TimeEntries/team', queryParameters: {
